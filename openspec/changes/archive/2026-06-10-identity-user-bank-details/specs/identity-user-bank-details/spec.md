...
The system SHALL store the sender bank IBAN on the authenticated Identity user profile.

#### Scenario: User has bank IBAN
- **WHEN** an Identity user has filled a bank IBAN
- **THEN** the system SHALL persist the IBAN with that Identity user

#### Scenario: Different users have different bank IBANs
- **WHEN** two Identity users fill different bank IBAN values
...
- **THEN** the system SHALL show the current bank IBAN value for that user

#### Scenario: User saves bank IBAN
- **WHEN** a signed-in user submits a valid bank IBAN on their account/profile page
- **THEN** the system SHALL save the IBAN to that user's Identity profile

#### Scenario: User cannot edit another user's bank IBAN
- **WHEN** a signed-in user updates bank details
- **THEN** the system SHALL update only the authenticated user's Identity profile

### Requirement: Bank account name comes from user fullname
The system SHALL use the authenticated sender's Identity user fullname as the bank account recipient name for monthly statement emails.

#### Scenario: Sender profile has fullname
- **WHEN** a monthly statement email is sent by a user whose Identity profile has a fullname
- **THEN** the payment recipient name SHALL be the sender's Identity user fullname

#### Scenario: Sender profile has no fullname
- **WHEN** a monthly statement email is sent by a user whose Identity profile has no fullname
...
The system SHALL NOT send a monthly statement email when the authenticated sender has no bank IBAN configured.
